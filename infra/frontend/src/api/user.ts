...
const findById = async (id?: string | number): Promise<User> => {
...
const update = async (id?: string | number, updateRoleDto?: UpdateUserDto): Promise<User> => {
  const response = await axios.put<User>(`public/user/${id}`, updateRoleDto);
  return response.data;
};

const deactivate = async (id?: string | number): Promise<User> => {
  const response = await axios.put<User>(`public/user/deactivate/${id}`);
  return response.data;
};

const activate = async (id?: string | number): Promise<User> => {
