...
export default function UserMain({ className }: UserMainProps) {
  ...
  const { mutate: activateUser, isPending: isActivationPending } = useMutation({
    mutationFn: (id?: string | number) => api.user.activate(id),
    onSuccess: () => {
      refetchUsers();
  ...
  const { mutate: deactivateUser, isPending: isDeactivationPending } = useMutation({
    mutationFn: (id?: string | number) => api.user.deactivate(id),
    onSuccess: () => {
      refetchUsers();
  ...
  const handleCreateSubmit = () => {
    ...
    if (!result.success) {
      ...
      const payload = result.data as unknown as CreateUserDto;
      createUser(payload);
    }
  };
  ...
  const handleUpdateSubmit = () => {
    ...
    if (!result.success) {
      ...
      updateUser({ id, user: payload } as any);
    }
  };
